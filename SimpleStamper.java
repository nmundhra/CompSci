 ...
 * SimpleStamper.java
 * 
 * consider a JPanel that does the following: Clicking on the panel with the left mouse button will place a red rectangle on the panel
 *  at the point where the mouse was clicked. Clicking with the right mouse button will place a blue oval on the panel. 
 * Holding down the Shift key while clicking will clear the panel by removing all the shapes that have been placed.
 * 
 */
import java.awt.*;
...
public class SimpleStamper extends JPanel implements MouseListener {
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if ( evt.isMetaDown() ) {
			...
			g.fillRect( x - 30, y - 15, 40, 40);
			g.setColor(Color.BLACK );
			g.drawRect( x - 30, y - 15, 40, 40);
		}
        else {
